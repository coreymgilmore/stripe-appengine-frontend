...
func Report(w http.ResponseWriter, r *http.Request) {
	...
	//check if report is in memcache
	//generate unique hash to identify this report data
	c := 		appengine.NewContext(r)
	hash := 	createMemcacheKey(datastoreId + startString + endString + hoursToUTC)
	var result reportData
	_, err := memcache.Gob.Get(c, hash, &result)
	
	//report is not in memcache
	...
	if err == memcache.ErrCacheMiss {
		...
		//save to memcache
		memcacheutils.Save(c, hash, result)
	}
...
//GENERATE HASH FOR MEMCACHE KEY
//so key is random and unique to the input value
func createMemcacheKey(input string) string {
	hasher := sha256.New()
